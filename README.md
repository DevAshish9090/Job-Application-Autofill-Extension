...
A Chrome/Brave extension backed by an AI-powered FastAPI service. It turns a one-time resume
upload into a reusable candidate profile, then intelligently fills application forms across job
portals, handling the messy real-world cases that most autofillers choke on. An LLM does the
heavy lifting: parsing resumes, matching unfamiliar fields, scoring job fit, and drafting answers.
...
<p>
  ...
  <img src="https://img.shields.io/badge/AI-Groq%20LLM-F55036?style=for-the-badge" alt="Groq LLM" />
...
- **Resume &rarr; profile.** Upload a PDF/DOCX and an AI model extracts your details into a structured, editable profile.
...
- **AI fallback matching.** Fields the built-in rules can't place are mapped to the right profile key by the model.
- **Saved answers memory.** Answer a screening question once (notice period, "how did you hear about us"), and it fills on future forms by meaning. Matching runs locally, so nothing leaves your machine.
...
- **Cover letters and AI answers.** Generate grounded, specific answers and cover letters from your profile and the job.
- **Application tracking and analytics.** Save each application with a status, then see totals, a funnel, and response rates by role.
- **Privacy first.** Your profile, saved answers, and application history live in the browser (`chrome.storage.local`). Backup, restore, and reset in one click.
...
| storage- profile + history  |          | /match-fields   AI matching  |
|          (local, private)   |          +---------------+--------------+
+-----------------------------+                          | Groq LLM
```
...
**Backend:** Python, FastAPI, Groq LLM, PyMuPDF and python-docx (resume parsing).
...
Get a free key at [console.groq.com](https://console.groq.com). Open `http://localhost:8000/health` and confirm `key_set` is `true`.
...
> Load unpacked is free and permanent for personal use, so no Chrome Web Store fee is required.
...
- Fully custom widgets (e.g. Workday) and cross-origin iframes are the hard edge, so coverage there is partial.
...
Released under the [MIT License](LICENSE). Personal and portfolio project, not affiliated with any job portal.
